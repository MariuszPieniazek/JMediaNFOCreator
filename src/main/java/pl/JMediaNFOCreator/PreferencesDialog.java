...
public class PreferencesDialog extends Dialog {
	 ...
	 */
	public PreferencesDialog(Shell parent, int style, TableColumn aTblClmnChecksum) {
		super(parent, style);
	...
	public Object open() {
        ...
        shellPreferencesDialog.setLocation(shellParentBounds.x + (shellParentBounds.width - shellDialogSize.x) / 2, shellParentBounds.y + (shellParentBounds.height - shellDialogSize.y) / 2);
	...
	private void createContentsPreferencesDialog(Composite parent) {
		...
		cTabFolderPreferences = new CTabFolder(shellPreferencesDialog, SWT.BORDER);
		...
		Composite compositePreferencesDialog = new Composite(shellPreferencesDialog, SWT.NONE);
		{
			FormLayout layout = new FormLayout();
			layout.marginTop = 5;  
			layout.marginLeft = 5;  
		...
		Button btnPreferencesOK = new Button(compositePreferencesDialog, SWT.PUSH);
	...
	protected void handleBtnPreferencesOKSelected(final SelectionEvent e) {
		try {		
			...
			FileOutputStream outputStream = new FileOutputStream(settingsDirectory+File.separator+"JMediaNFOCreator.xml");
			Properties prop = new Properties();
			if (btnGenerateSumOfControl.getSelection()==false) {
				prop.setProperty("autocrc", "0");
			}
			else prop.setProperty("autocrc", "1");
			...
			else prop.setProperty("addbitrate", "1");
		    if (btnWriteUnicodeFilenames.getSelection()==false) {
				prop.setProperty("saveunicode", "0");
			}
			else prop.setProperty("saveunicode", "1");
		    prop.setProperty("crc", comboGenerateSumOfControl.getText());
		    prop.storeToXML(outputStream, "Konfiguracja programu JMediaNFOCreator");
		    outputStream.close();
		...
		if (btnGenerateSumOfControl.getSelection() == false) tblClmnChecksum.dispose(); 
		shellPreferencesDialog.dispose( );
	}
	...
	public void generalCTab() {
		...
		Composite compositeGeneral = new Composite(cTabFolderPreferences, SWT.NONE);
		{
			FormLayout layout = new FormLayout();
			layout.marginTop = 5;  
			layout.marginLeft = 5;  
		...
		btnAddBitrate.setText(Messages.PD_btnAddBitrateTxt);
		{
			...
			if (new FilesOperations().getCfgProperty("JMediaNFOCreator.xml", "addbitrate").equals("1")) {
				btnAddBitrate.setSelection(true);
			}
			else btnAddBitrate.setSelection(false);
		}
		...
		btnGenerateSumOfControl.setText(Messages.PD_btnGenearteSumOfControlTxt);
		{
			...
			else btnGenerateSumOfControl.setSelection(false);
		}
		...
		comboGenerateSumOfControl = new Combo(compositeGeneral, SWT.LEFT);
		{
			...
			if (new FilesOperations().getCfgProperty("JMediaNFOCreator.xml", "crc").equals("MD5")) {
				comboGenerateSumOfControl.select(1);
			}
			else comboGenerateSumOfControl.select(0);
		}
		...
		txtDefaultDir = new Text(compositeGeneral, SWT.BORDER);
		{
			...
			txtDefaultDir.setText(new FilesOperations().getCfgProperty("JMediaNFOCreator.xml", "defaultpath"));
		...
		btnWriteUnicodeFilenames.setText(Messages.PD_btnWriteUnicodeFilenames_Txt);
		{
			FormData data = new FormData();  
			data.left = new FormAttachment(0);  
			data.top = new FormAttachment(lblDefaultDir, 10);
			btnWriteUnicodeFilenames.setLayoutData(data);
			if (new FilesOperations().getCfgProperty("JMediaNFOCreator.xml", "saveunicode").equals("1")) {
				btnWriteUnicodeFilenames.setSelection(true);
			}
			else btnWriteUnicodeFilenames.setSelection(false);
		}
	...
	public void languageCTab() {
		...
		Composite compositeLanguage = new Composite(cTabFolderPreferences, SWT.NONE);
		{
			FormLayout layout = new FormLayout();
			layout.marginTop = 5;  
			layout.marginLeft = 5;  
		...
		listLanguages = new List(grpLanguage, SWT.LEFT | SWT.V_SCROLL | SWT.BORDER);
		{
			FormData data = new FormData();  
			data.left = new FormAttachment(0);
			...
			String selectedDirectoryLang = this.getClass().getResource("/lang").getPath();
			if (selectedDirectoryLang != null) {
				String[] filterFilesLang = new DirList().getDirList(".*"+File.separatorChar+".lng", selectedDirectoryLang);
				if (filterFilesLang.length > 0) listLanguages.setItems(filterFilesLang);
			}
		}
		...
		lblDisplayChoosenLangauage = new Label(grpLanguage, SWT.BORDER);
		{
			...
			lblDisplayChoosenLangauage.setText(new FilesOperations().getCfgProperty("JMediaNFOCreator.xml", "language"));
		}
		...
		Text txtInfoAboutTranslate = new Text(grpLanguage, SWT.BORDER | SWT.WRAP);
		txtInfoAboutTranslate.setText(Messages.PD_txtInfoAboutTranslate_text);
		{
			...
			data.bottom = new FormAttachment(lblDisplayChoosenLangauage, 0, SWT.BOTTOM);
	...
	protected void handleBtnChooseLanguageSelected(final SelectionEvent e) {
		lblDisplayChoosenLangauage.setText(listLanguages.getItem(listLanguages.getSelectionIndex()));
	}
